...
状态：**已按本版实施**（管理员代配 API + 管理页 + 禁本地下发）。产品补充决定：代配的 LLM Key **暂不允许**下发到试用用户本机 Obsidian；用户自行更换 Key 后可绑定。B 站登录态仅云端使用。
...
| 本地 Obsidian | **首版不下发代配 Key**：代配 LLM 配置标记 `local_export=denied`，本机绑定接口拒绝。用户自行填写 Key 后可绑定。后续再评估是否放开。 |
...
| 凭据 | 云端 | 本地插件 | 说明 |
| --- | --- | --- | --- |
| LLM Key | enrich 使用 | **暂不可**（`local_export=denied`） | 管理员代配后本机绑定 403；用户自行 PATCH secret 后改为允许绑定。 |
