...
| POST `/v1/items/{id}/source-text` | 编辑原文 | 一行一块（`#` 开头为标题）；`expected_source_revision` 冲突 409；编辑后文本成为新不可变来源版本，旧版本保留；内容无变化不新增版本；按自动加工开关重新排队（「AI 自动整理」与「AI 自动纠错与分段」各自独立，任一开着就入队） |
...
| GET/PUT/DELETE `/v1/platform-sessions/{platform}` | 小红书/视频号/知乎登录态（Cookie 文本或字段）托管 | 与 B 站同一套信封加密与「只进不出」规则；`POST .../test` 可探测；明文不回读；更新后同平台待登录条目自动重排队 |
...
| GET/PATCH `/v1/settings` | 默认处理配置 | 只接受非秘密设置；版本检查。`auto_enrich`（AI 自动整理，产出知识笔记）与 `ai_paragraphing`（AI 自动纠错与分段，只改写阅读层文字）是两个独立开关，默认开：任一开着提取完成后就入 enrich 任务，任务内部按各自开关决定做哪一半；都关则停在 extracted 等手动「开始整理」 |
...
- `optimize` 优化文本：负责纠错与分段（「AI 自动纠错与分段」的调用），建议关闭思考模式、配轻量模型；未配置或凭据不可用时回退整理模型。

Provider 的 `endpoint + model + 参数` 必须来自同用户已保存配置，不能由外部来源正文、模型返回文本或 Capture 任意覆盖。不给模型工具权限，模型不能发网络请求、调用 shell 或选择其他 API Key。

### 11.2 输出字段

内容协议 v3（字段级定义见 docs/24 §1、§3）：模型只写内容和选本次给它的 `R` 编号，
文档身份、来源版本、引用表和哈希由程序组装，模型不回填。

- `title`、`summary`：标题与一句话总结。
- `sections[]`：`{heading, blocks[]}`；不分配章节或内容块 ID，顺序即数组顺序。
- `blocks[].kind`：`claim`（来源主张，必须有依据）、`quote`（逐字摘录，必须有依据）、
  `suggestion`（AI 候选启发，可无依据）、`text`（导语与结构说明）。
- `blocks[].refs`：本次 `material` 里的 `R` 编号；程序查表换成文档内的 `e` 键并绑定
  固定来源版本与原文范围。
- `limitations[]`：原文缺失、OCR 可疑、字幕未含视觉信息等。
- 对话/工作流材料不再输出独立 `workflow` 结构：问题、约束、提出/接受/否定/未知、被放弃
  方案与结果按内容写成自然章节，状态含义不得丢失。
  ...
  "sections": [
    {
      "heading": "主要判断",
      "blocks": [
        {"kind": "claim", "text": "内容接收和总结应分开处理。", "refs": ["R1"]},
...
知识融合任务在此基础上增加 `no_op`、`change_summary`、`conflicts`；分享整合增加
`reader_goal`、`visualization_intent`、`material_usage`。云端不输出主题、标签、知识关联、
晋升判断或 Obsidian 链接；作者、发布日期和来源 URL 由可靠元数据或用户提交填充，未知为 null。
长材料分块汇总时，模型只能沿用已通过校验候选里的引用，摘录必须整条沿用，不得自己重新摘取或
把改写句升级为逐字摘录。
