...
| POST `/v1/items/{id}/source-text` | 编辑原文 | 一行一块（`#` 开头为标题）；`expected_source_revision` 冲突 409；编辑后文本成为新不可变来源版本，旧版本保留；内容无变化不新增版本；按自动加工开关重新排队（「AI 自动整理」与「AI 语义分段与纠错」各自独立，任一开着就入队） |
...
| GET/PATCH `/v1/settings` | 默认处理配置 | 只接受非秘密设置；版本检查。`auto_enrich`（AI 自动整理，产出知识笔记）与 `ai_paragraphing`（AI 语义分段与纠错，只改写阅读层文字）是两个独立开关，默认开：任一开着提取完成后就入 enrich 任务，任务内部按各自开关决定做哪一半；都关则停在 extracted 等手动「开始整理」 |
