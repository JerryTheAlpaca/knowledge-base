...
- 入口在「已收集」抽屉的工具栏：按钮叫「分享」，按下去每行右侧出现一个圆框表示勾选
  （行右侧那条带平时就留着，进出多选文字都不横向跳），再按「分享」或 Esc 退出多选。
  底部条显示已选数量 + 「完成」；单篇详情保留「用这篇生成分享页」入口。
...
- 创作页直接搭在首页之上（`#shareStage`）：金蔷薇盖一层**只压暗、不加模糊**的强遮罩
  ...
  输入框，右上角叉叉退出（Esc、浏览器返回同效）；顶栏改成三横线，进去是以前的所有
  对话（标题 + 状态 + 版本 + 时间），点开继续。
...
| 2026-09-21 多选圆框与「完成」三选一 | 同一套打桩走查：进多选时行内文字右缘不动（998→998）、圆框 22px 空心/金底打勾两态；「完成」菜单在 390 与 1280 视口都贴在底条上方 10px 且不越界；勾 3 篇 → 下载整理稿得到 2 个 `-整理稿.md`（缺 `preview.md` 那篇被跳过并在提示里报数），下载原文另登 `source-download`；「制作 HTML」进舞台草稿；Esc 逐级收菜单/舞台/多选；无 JS 报错 |
