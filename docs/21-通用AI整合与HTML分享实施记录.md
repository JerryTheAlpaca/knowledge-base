...
- 入口在「已收集」抽屉的工具栏：按钮就叫「分享」，按下进入多选（翻页/筛选不清空、
  localStorage 持久、底部条显示已选数量、清空/退出/开始创作），单篇详情保留
  「用这篇生成分享页」入口。
- 创作页直接搭在首页之上（`#shareStage`）：金蔷薇盖一层强遮罩退成背景，中间是多轮
  对话流，底部复用首页那副胶囊输入框，右上角叉叉退出（Esc、浏览器返回同效）；
  顶栏改成三横线，进去是以前的所有对话（标题 + 状态 + 版本 + 时间），点开继续。
