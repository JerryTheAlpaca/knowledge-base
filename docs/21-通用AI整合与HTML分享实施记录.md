...
## 6. 2026-09-22 创作对话界面改版

对着成熟 Agent 工具的交互习惯重排了分享创作舞台（`inbox.html` 的 `#shareStage`、`js/shares.js`、`inbox.css` 舞台段），四件事：

- **问答卡移到输入框上方**：待答的一组不再铺在对话流里，改画进 `.stage-dock` 新增的 `#stageAsk`，一题一题答，位置固定不用回滚。整块 innerHTML 重画的旧问题随之消失——切题只重画这一张卡，不再重画对话流。
- **答完自动跳下一题**：选中预设选项即算答完本题并跳到后面第一道没答的，焦点落到新题的第一个选项；改已经答过的题不跳。点「其他」把焦点交给补充输入框、不跳，写完回车再往后走。整组答完时底部「已答 2 / 3」翻成绿色的「都答完了」并换成主提交按钮。打字过程中只改底部那一行和进度点，不重画整张卡，否则会打断输入。
- **需求摘要移到顶部**：`#stagePlan` 折成一行也留「作品目的／给谁看」摘要，展开两栏排布；到「请确认方向」这一步自动摊开一次。
- **答完的轮次折起来**：按 `round_id` 把结构化回答配回提问那一组，折成「问了 3 个问题 ＋ 当时选了什么」一行；点开高亮选中项。为此 `GET /runs/{id}/conversation` 的用户消息补了 `free_text`，你这一侧的气泡只留自己打的那句话，不再和摘要重复一遍。
- **过程信息**：对话流末尾一块「过程」，按真实 `run.stage` 列出五步、标出当前那步，跑动时摊开并计时，停下自动折回一行。计时只累计被观察到在跑的那段（`run.created_at` 含着用户思考的几分钟，拿来当「处理了多久」会把等待算成干活）。输入框上方那条状态胶囊撤掉，只留需要看一眼的原因，阶段文字由过程块说。

本轮实际做过的验证（`.workbuddy/share_ui_server.py` 起假模型服务，走通多选材料 → 两轮问答 → 确认方向）：连接浏览器里驱动问答卡，量到第 1→2→3 题自动跳转、焦点落在新题首个选项、进度点与「已答 N / 3」同步、选「其他」不跳而写字后回车跳、打完三题按钮翻成「提交这 3 个回答」；提交后问答卡收起、该轮折成一行且三个选项按 `option_ids` 标成选中态、「其他」的原话跟在下面；确认方向那一步顶部方案自动摊开；过程块在 `awaiting_runner` 下展开并逐秒走表（「排队中 · 检查页面 · 已用 31→35 秒」），点停止后自动折回、文案换成「用时」。无头 Edge 出了四张状态截图核对排版。

未跑测试：本轮是前端布局与文案重排 ＋ 一个只增字段的响应改动，没有触碰隔离、凭据、预算或投递逻辑。`free_text` 是往已有 `content` 里多读一个早就写进去的键，服务端行为不变。
