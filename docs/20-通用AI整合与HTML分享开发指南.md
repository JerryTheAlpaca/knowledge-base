...
- 列表新增“选择”模式，勾选一篇或多篇后显示“生成分享页”。单篇详情提供同名入口。
- 选择集合按 Item ID 保持，翻页或筛选不悄悄清空；显示已选数量，可展开查看、移除。
- 打开创作面板：上方是选中的材料，下方只有一个主要要求输入框和“开始创作”。点击后进入需求对话，不立即生成整页代码。
- 输入框提示：“想把这些材料做成什么？可以说内容重点、读者、呈现方式或交互要求。”
...
对话旁显示可展开的“目前的需求”，用普通语言记录：作品目的、读者、内容重点、材料处理方式、呈现偏好、必须保留／避免的内容，以及 AI 暂时采用的假设。不要求用户逐字段编辑或逐条批准。

AI 判断信息已经足够时，给出简短复述和拟采用的表达方向，显示“确认并生成”和“再调整一下”。用户确认的是一份可见的需求摘要；不再额外要求逐级批准提纲、图表和代码。
...
- 私有预览包含页面、来源列表、继续提要求输入框、“下载 HTML”和“分享”。
