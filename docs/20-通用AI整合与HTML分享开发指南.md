...
- 「已收集」列表工具栏提供“分享”按钮进入多选，勾选一篇或多篇后底部显示“开始创作”。单篇详情提供“用这篇生成分享页”入口。
- 创作页直接复用首页结构：金蔷薇压在一层强遮罩后面当背景，中间是与 AI 的多轮对话，底部沿用首页那副胶囊输入框，像一个 Agent 工具。右上角叉叉退出（Esc 与浏览器返回同样退出）；顶栏的三横线是以前所有对话的列表，点开可以继续。
