...
- 「已收集」列表工具栏提供“分享”按钮进入多选：进入后每行右侧出现一个圆框表示勾选（行右侧那条带平时就留着，进不进多选文字都不横向移）；再按一次“分享”或 Esc 退出多选。单篇详情提供“用这篇生成分享页”入口。
...
- 多选完点“完成”，就地弹一个小菜单三选一：下载原文、下载整理稿、制作 HTML。前两项逐条取当前 Bundle 里的 `readable.md`（回退 `normalized.md`）与 `preview.md`，按条目标题存成 `.md`；这份文件不存在的条目跳过，并在提示里说清几篇没有。下载原文沿用单篇详情的做法，顺带登记 `source-download` 把条目推到“已下载”。“制作 HTML”才进下面这套对话。
...
对话末尾以一张卡片显示“目前的需求”，用普通语言记录：作品目的、读者、内容重点、材料处理方式、呈现偏好、必须保留／避免的内容，以及 AI 暂时采用的假设。不要求用户逐字段编辑或逐条批准。
...
只显示当前阶段，不编造进度百分比。关闭页面不取消任务；重新打开能恢复状态。提供“停止生成”，停止后不再发起新的模型调用，已发出的调用不承诺能撤回或免计费。
