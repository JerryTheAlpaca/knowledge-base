// touch-bloom.js — 点一下花冠：指尖迸一小片金粉，手下那一块花瓣从触点往外一层层漫开
//
...
export function attachBloomTouch(svg) {
  ...
  // 一层自己慢慢胀出去、停一停、再更慢地落回；几层之间按离手指的远近错开半拍，
  // 于是一口气是从触点往外漫的，不是整朵齐刷刷弹一下（齐刷刷就是"生硬"的来源）。
  // 峰值只比首屏那圈 13s 的常驻呼吸（2.4%）高一点，看着像花自己开大了，不像被拽动。
  function nudge(ring, k, step) {
    if (ring.anim) ring.anim.cancel();
    ring.anim = ring.layer.animate([
      { transform: "scale(1)", easing: "cubic-bezier(.22, .58, .32, 1)" },
      { offset: 0.38, transform: `scale(${(1 + 0.036 * k).toFixed(4)})`,
        easing: "cubic-bezier(.38, .02, .55, 1)" },
      { transform: "scale(1)" },
    ], { duration: 1900, delay: step * 130 });
  }
  ...
  function flash(x, y) {
    ...
    el.animate([
      { opacity: 0, transform: "scale(.5)", easing: "cubic-bezier(.2, .6, .35, 1)" },
      { opacity: .85, transform: "scale(1)", offset: 0.34, easing: "cubic-bezier(.4, .1, .6, 1)" },
      { opacity: 0, transform: "scale(1.32)" },
    ], { duration: 950 }).addEventListener("finish", () => el.remove());
  }
  ...
  function burst(x, y) {
    ...
    for (let i = 0; i < n && sparks < MAX_SPARKS; i++) {
      ...
      const d = 10 + Math.random() * 22;   // 撒得近：粉是飘开的，不是喷出去的
      const dx = Math.cos(a) * d;
      const dy = Math.sin(a) * d * 0.8 - 8 - Math.random() * 9;
      p.animate([
        { transform: "translate(0,0) scale(.4)", opacity: 0, easing: "cubic-bezier(.3, .45, .3, 1)" },
        { transform: `translate(${(dx * 0.55).toFixed(1)}px,${(dy * 0.55).toFixed(1)}px) scale(1)`, opacity: 1, offset: 0.3,
          easing: "cubic-bezier(.4, .1, .6, 1)" },
        { transform: `translate(${dx.toFixed(1)}px,${dy.toFixed(1)}px) scale(.55)`, opacity: 0 },
      ], { duration: 980 + Math.random() * 380, delay: i * 14, fill: "forwards" })
  ...
  svg.addEventListener("click", (e) => {
    ...
    for (const ring of geo.rings) {
      const step = Math.abs(ring.at - hit.at);
      const band = Math.max(0, 1 - step / 3.4);
      const near = 1 - Math.min(1, Math.abs(ring.reach - r) / (geo.gap * 2));
      const w = band * (0.55 + 0.45 * near);
      if (w > 0.06) nudge(ring, w, step);
    }
